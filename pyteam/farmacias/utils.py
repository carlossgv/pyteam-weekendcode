...
def devuelve_farmacias_cercanas(
    usuario_latitud: float, usuario_longitud: float, url: str
) -> list:
    """
    ...
    for farmacia in farmacias:
        ...
        if check_latitud.isdigit() and check_longitud.isdigit():
            ...
            coordenada_farmacia = (
                float(farmacia["local_lat"]),
                float(farmacia["local_lng"]),
            )
            if (
                distance.distance(coordenada_farmacia, coordenada_usuario).km
                <= DISTANCIA_MINIMA
            ):
